...
def help_message(): # Print the help message
    help_message = '''
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
HashTag: A Hash Cracking Script

    Author: John Hilde
    Source: https://github.com/john-hilde/hashtag
    Version: 1.0
    Release Date: 20230627
    
    Special Thanks to Zion3R for the creation of the hash-id.py! His code was used in order to get the correct format of the hash!
        Source: https://github.com/blackploit/hash-identifier/blob/master/hash-id.py
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Usage: 

    hashtag [-w WORDLIST] [-p HASHFILE] [-f FORMAT] [-show] [-b BRUTE_FORCE] [-bb FULL_BRUTE_FORCE] [-k KEEP] [hash]
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Positional Arguments:

    hash                            The single hash value to crack
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Options:
...
Formats:
    ...
    MD5, SHA-1, SHA-256, SHA-224, SHA-384, SHA-512
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Example usage:

    hashtag -w wordlist.txt -p hashes.txt
        Finds the worldists in the common directory. Then uses those passwords to crack the hashes in hashes.txt
        
    hashtag -w /usr/share/wordlist/rockyou.txt -f MD5 -p hashes.txt
        Finds the worldlist at /usr/share/wordlist/rockyou.txt. Then forces the encryption format to be MD5. Finally, running it against every line of thr hashes.txt
    
    hashtag -w /usr/share/wordlist/rockyou.txt 1A732667F3917C0F4AA98BB13011B9090C6F8065 
        Finds the worldlist at /usr/share/wordlist/rockyou.txt. Then uses those passwords to crack the hash: 1A732667F3917C0F4AA98BB13011B9090C6F8065
        
    hashtag -b kang@@@@ 1A732667F3917C0F4AA98BB13011B9090C6F8065
        Creates a brute-force worldist (kangaaaa - kangzzzz). Then uses those passwords to crack the hash: 1A732667F3917C0F4AA98BB13011B9090C6F8065
...
def hash_identifier(hash): # hash-id.py script by Zion3R (https://github.com/blackploit/hash-identifier/blob/master/hash-id.py)
    first = None
...
# hash.isalnum()  alfanumerico
    ...
    while True:
        ...
        first = None
        hash_type_list = [algorithms[jerar[0]], algorithms[jerar[1]]]
        print(f"\nPossible Hashes: {hash_type_list}")
        return hash_type_list

def generate_wordlist(brute_force): # Generates the wordlist based on the brute force string
    ...
    # Generate a unique filename for the wordlist
    counter = 1
    while True:
        filename = f"wordlist_{counter}.txt"
...
def crack_hash(hash_value, wordlist, hash_type_length): # Default implementation for cracking hashes
    ...
    for i in range(len(hash_type_length)): # Goes through the list of hash types, or just one, that were found or specified
        hash_type = hash_type_length[i]
        print(f"Hash type: {hash_type}\n")

        # Dictionary of all possible hash functions
        hash_functions = {
            "MD5": hashlib.md5,
            "SHA-1": hashlib.sha1,
            ...
            #"Whirlpool": hashlib.whirlpool,
            
            # Add more hash types as needed
        }

        if hash_type not in hash_functions:
            print("\n\033[91mNo Tag..\033[0m")
            return ['Not Cracked', 'Invalid Hash Type']

        hash_function = hash_functions[hash_type]

        for encoding in encodings:
            try:
                with open(wordlist, 'r', encoding=encoding) as wordlist_file:
                    for password in wordlist_file:
                        password = password.strip()
                        hashed_password = hash_function(password.encode(encoding)).hexdigest()  # Hash the password using the selected hash function and encoding                     
                        if hashed_password.lower() == hash_value.lower() or hashed_password.upper() == hash_value.upper(): #  # Check if the hashed password matches the provided hash value
                            print("\033[92mTag!\033[0m")
                            return password, hash_type
            except UnicodeDecodeError:
                continue

    print("\033[91mNo Tag..\033[0m") # Shucks!
    return ['No Password Found', 'No Hash Found']

def brute_force_crack_hash(hash_value, hash_type_length, max_length_password): # Full-Brute force implementation for cracking hashes
    print(f"Cracking hash: {hash_value}")

    encodings = ["utf-8", "latin-1"]  # Add more encodings as needed

    brute_force_chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()" # Brute force characters to be used in the brute force cracking process
    ...
    for i in range(len(hash_type_length)): # Goes through the list of hash types, or just one, that were found or specified
        ...
        if hash_type not in hash_functions: # Check if the hash type is valid
            print("\n\033[91mNo Tag..\033[0m")
            return ['Not Cracked', 'Invalid Hash Type']

        hash_function = hash_functions[hash_type]

        password_length = 4 # Starting password length
        password_found = False # Flag to check if a password has been found
        max_length_password = int(max_length_password) # Changes it to an int

        while not password_found:
            combinations = itertools.product(brute_force_chars, repeat=password_length) # Generate all combinations of brute force characters with the current password length
            for combination in combinations:
            ...
            password_length += 1
            if password_length > max_length_password:  # Sets a maximum password length (To avoid infinite loops and overheating the system)
                break

    print("\033[91mNo Tag..\033[0m")
    return ['No Password Found', 'No Hash Found']

def display_password(password, hash): # Displays the password and the format of the hash
    print("\033[93m" + "-" * 80 + "\033[0m")
    print(f"HASH: {hash}\nPassword: \033[1m{password[0]}\033[0m\nFormat: {password[1]}")
    print("\033[93m" + "-" * 80 + "\033[0m")  
...
def main(): # The Bread and Butter
    parser = argparse.ArgumentParser(description='Hash Cracking Script', formatter_class=argparse.RawTextHelpFormatter) 
    parser.add_argument('-w', '--wordlist', help='Path to the wordlist file')
    parser.add_argument('-p', '--hashfile', help='Path to the file containing hashes')
    parser.add_argument('hash', nargs='?', help='The single hash value to crack')
    parser.add_argument('-f', '--format', help='The Hash format to crack')
    parser.add_argument('-show', action ='store_true', help='Display help information')
    parser.add_argument('-b', '--brute', help='String for brute-force')
    ...
    if args.update: # If the user wants to update the script
        update_script()
        sys.exit(0)
    ...
    if args.hash is None and args.brute is not None: # If the user wants to create JUST a brute-force password wordlist
        ...
        generate_wordlist(args.brute)
        print("\033[92mDone!\033[0m\n")
        sys.exit(0)   
        
    if args.brute: # If the user wants to create a brute-force password wordlist
        if args.format is None:
            args.format = hash_identifier(args.hash)
        print("\n\033[91mGenerating brute-force wordlist...\033[0m")
        wordlist = generate_wordlist(args.brute)
        print("\033[92mDone!\033[0m\n")
        password = crack_hash(args.hash, wordlist, args.format)
        display_password(password, args.hash)
        if args.keep != True:
            print("Deleting wordlist...\n")
            os.remove(wordlist)
        sys.exit(0)
    elif args.full_brute: # If the user wants to do a brute-force, but does NOT want to create a brute-force wordlist
        if args.format is None:
            args.format = hash_identifier(args.hash)
        password = brute_force_crack_hash(args.hash, args.format, args.full_brute)
        display_password(password, args.hash)
    elif args.hashfile is not None: # If the user wants to crack a hash file
        with open(args.hashfile, 'r', encoding='utf-8') as hash_file:
            try:
                for hash_value in hash_file:
                    hash_value = hash_value.strip()
                    if args.format is None:
                        args.format = hash_identifier(hash_value)
                    password = crack_hash(hash_value, args.wordlist, args.format)
                    display_password(password, args.hash)
            except UnicodeDecodeError:
                hash_file.seek(0)  # Reset file pointer
                for hash_value in hash_file:
                    hash_value = hash_value.strip()
                    if args.format is None:
                        args.format = hash_identifier(hash_value)
                    password = crack_hash(hash_value, args.wordlist, args.format, encoding='latin-1')
                    display_password(password, args.hash)
    elif args.hash is None and args.wordlist is None:
        print("\033[94mUsage: hashtag [-w WORDLIST] [-p HASHFILE] [-f FORMAT] [-show] [-b BRUTE_FORCE] [-bb FULL-BRUTE-FORCE] [-k KEEP] [hash]\033[0m\n")
    else: # If the user wants to crack a single hash value
        if args.format is None:
            args.format = hash_identifier(args.hash)
            
        password = crack_hash(args.hash, args.wordlist, args.format)
        display_password(password, args.hash)

if __name__ == '__main__': # Main functions
    clear_screen()
    logo()
    main()
